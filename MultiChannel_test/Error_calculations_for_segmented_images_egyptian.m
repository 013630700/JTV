...
% egyptian phantom
load XRsparse_aTV_JTV_egyptian recn1 recn2 target1 target2 obj
load JTV_egyptian_for_segmentations im1 im2 im3 im4 N
% % figure(1)
...
% egyptian TRESHOLD: 0.3354
im3_thresholded = recn1;
im3_thresholded(im3>0.4599) = 1;
im3_thresholded(im3<0.4599) = 0;
...
% egyptian TRESHOLD: 0.302
im4_thresholded = recn2;
im4_thresholded(im4>0.31) = 1;
im4_thresholded(im4<0.31) = 0;
nnz(im4_thresholded)
% display result
% We want to treshold the target1 also to be zero or one.
...
% Calculate how many pixels were misclassified in reconstruction 1. 
% Each pixel has either value zero or one, so it enought to check how many
% pixels in the difference were non-zero.
...
axis square;
axis off;
title({'egyptian1 phantom, Ground truth'});
...
axis square;
axis off;
title({'egyptian2 phantom, Ground truth '});
...
Image = getframe(gcf);
imwrite(Image.cdata, 'segmentations_egyptian_JTV.jpg');
...
save color_segmentations_JTV_Egyptian pha pha1 pha2 im im1 im2